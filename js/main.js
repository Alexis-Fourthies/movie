...
async function loadMovies() {
  try {
    const response = await fetch(`https://www.omdbapi.com/?s=${encodeURIComponent(keyWords.value)}&apikey=5865a16a`);
...
async function loadMovie(imdbID) {
  if (imdbID !== null && imdbID !== undefined) {
    try {
      const response = await fetch(`https://www.omdbapi.com/?i=${imdbID}&plot=full&apikey=5865a16a`);
      //console.log(response);
      if (response.ok) {
        const responseJson = await response.json();
